...
Documentation in dbt Cloud refers to an automatically generated, interactive data catalog that provides detailed information about the models, sources, columns, tests, seeds, lineage (data flow). The documentation is built from the `descriptions` field added add to the project via .yml files. Once generated, they become part of a data catalog with an interactive UI.

To generate in dbt Cloud, we need run the following command in the command line at the bottom of the page: 
```CLI
dbt docs generate
```

Then, the documentation site is built (*or updated*) and can be viewed it directly from the Docs tab in the dbt Cloud UI. In the below screenshot you can see the documentation generated for the `customer_visited_most_locations` model.
