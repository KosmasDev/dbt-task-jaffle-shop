...
11. [Create Custom Test](#-create-custom-test)
12. [Additional dbt features](#-additional-dbt-features)
    1. [Lineage](#lineage)
...
![image](https://github.com/user-attachments/assets/50052d73-d39e-4eba-a587-1a2c1c62a5be)
...
In the context of a dbt workflow, tests are a way to validate the data. They ensure our assumptions hold true. There are 2 categories of tests:
...
Below is the simple lineage diagram that was automatically generated by dbt Cloud for this project.

![image](https://github.com/user-attachments/assets/d3def913-842b-4a80-97c8-17877bc59aa9)

## Documentation

Documentation in dbt Cloud refers to an automatically generated, interactive data catalog that provides detailed information about the models, sources, columns, tests, seeds, and lineage (data flow). The documentation is built from the `descriptions` field added to the project via .yml files. Once generated, they become part of a data catalog with an interactive UI. For more information about dbt Documentation process, check the official documentation [link 1](https://docs.getdbt.com/docs/build/documentation) and [link 2](https://docs.getdbt.com/reference/commands/cmd-docs).

To generate in dbt Cloud, we need to run the following command in the command line at the bottom of the page: 
```CLI
dbt docs generate
```

Then, the documentation site is built (*or updated*) and can be viewed it directly from the Docs tab in the dbt Cloud UI. In the screenshot below, you can see the documentation generated for the `customer_visited_most_locations` model.

![image](https://github.com/user-attachments/assets/55215dee-8c85-4875-b789-98fb5d6598ed)
...
As part of this project, a custom macros file was created to demonstrate how dbt enables the building of reusable logic for testing and transformations. Although standard tests were already in place for the `most_loyal_customer_per_location` model, we implemented an additional custom `not_null` test using a macro to showcase the flexibility and power of dbt's extensibility features.

1️⃣ **Step 1**: Create an `.sql` file under the `macros/` folder. This is a reusable macro code.
...
2️⃣ **Step 2**: Create an `.sql` file under the `tests/` folder. This is a test targeting only the `most_loyal_customer_per_location` model.
...
3️⃣ **Step 3**: Run the following dbt command to run the test.

```CLI
dbt test --select most_loyal_customer_per_location
```

For more information about dbt Macros, check the [official documentation](https://docs.getdbt.com/docs/build/jinja-macros).

## Orchestration

Orchestration in dbt Cloud is a key building block. While we did not cover this topic in the context of this project, every dbt user needs to have a foundational understanding of how orchestration works. 
...
dbt Cloud supports both scheduled orchestration (*time-based using cron*) and event-driven orchestration (*via API, Git actions, or webhooks*), as well as CI jobs for validating changes during development. This orchestration framework simplifies pipeline automation and ensures consistent deployment of dbt projects across environments. For more information about dbt Orchestration, check the [official documentation](https://docs.getdbt.com/docs/deploy/deployment-tools).

## Hooks 

Hooks in dbt are pieces of SQL that you can execute before or after a model runs, or during different stages of the dbt lifecycle (like `on-run-start`, `on-run-end`). They’re super handy for managing warehouse behavior, like setting session variables, running logging tables, or even managing permissions. You can define hooks inside `dbt_project.yml` or as part of your model configurations. For more information about dbt Hooks, check the [official documentation](https://docs.getdbt.com/docs/build/hooks-operations).

## Exposures

Exposures are a metadata feature in dbt that allows you to document how models are used by downstream tools, like dashboards, reports, or notebooks. They help connect the dots between raw data transformations and how that data is ultimately consumed, giving visibility into dependencies and data lineage. This is useful for governance, impact analysis, and showing stakeholders where their data lives. For more information about dbt Exposures, check the [official documentation](https://docs.getdbt.com/docs/build/exposures).

## Snapshots

Snapshots in dbt are used to track slowly changing dimensions (SCDs), especially when you want to keep historical versions of your source data. For example, if a customer’s email or address changes, snapshots allow you to track those changes over time rather than overwriting the old values. You define a snapshot with logic that specifies how to detect changes, and dbt takes care of managing version history in a dedicated snapshot table. For more information about dbt Snapshots, check the [official documentation](https://docs.getdbt.com/docs/build/snapshots).

## Working with Jinja Variables

Jinja is the templating engine that powers dbt's logic — it lets you write dynamic SQL using variables, conditionals, loops, and macros. You can define variables in your dbt_project.yml or pass them in at runtime, then access them with var('my_var'). This is powerful for things like changing behavior between dev and prod environments, filtering data by time window, or building reusable logic inside macros. For more information about dbt Jinja variables, check the [official documentation](https://docs.getdbt.com/reference/dbt-jinja-functions/var).

## Debugging with Logging

When something goes wrong in your dbt project, dbt’s built-in logging and debugging tools can help you figure out why. You can use dbt debug to test your setup (like database connection issues), and logs (found in logs/dbt.log) provide detailed info about run times, errors, SQL statements, and context. Setting different log-levels (like debug, info, or warn) gives you control over the verbosity, which is key for diagnosing issues. For more information about dbt Jinja, check the official documentation [link 1](https://docs.getdbt.com/reference/commands/debug) and [link 2](https://docs.getdbt.com/reference/global-configs/logs).
